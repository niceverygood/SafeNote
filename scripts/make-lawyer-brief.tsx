  ...
  <Document title="세이프노트 법률 검토 요청 자료" author="SafeNote">
    ...
    <Page size="A4" style={s.page}>
      ...
      <Text style={[s.body, { marginBottom: 10 }]}>
        ...
        ① 면책 자가진단(의무 이행 갭 점검) ② 위험성평가 생성·확정 ③ 노동자용 현장 증빙(작업 전
        점검·위험 신고)으로 구성됩니다.
      </Text>
      ...
      <View style={s.card}>
        <Text style={s.h3}>4. 증빙 무결성</Text>
        <Text style={[s.body, s.muted]}>현장 기록(작업 전 점검·위험 신고)은 타임스탬프와 해시 체인(append-only)으로 저장해 사후 변경을 감지할 수 있도록 설계했습니다.</Text>
      </View>
      <Footer p="01 · 개요와 원칙" />
    </Page>
    ...
    <Page size="A4" style={s.page}>
      ...
      <View style={[s.card, { padding: 0 }]}>
        ...
        <Row a="제3호 유해·위험요인 확인·개선 절차" b="위험성평가 생성·확정, 작업 전 위험 고지·확인 기록, 위험 신고→개선 이력" />
        <Row a="제4호 안전보건 예산" b="(범위 밖) 별도 관리 — 진단 항목으로 점검" />
        <Row a="제5호 책임자 업무수행 점검" b="반기 점검·갱신 이력 누적" />
        <Row a="제7호 종사자 의견 청취" b="노동자 위험 신고·현장 체크 기록(실시간)" />
        <Row a="제8호 비상대응 매뉴얼" b="(로드맵) 매뉴얼·점검 기록화 예정" />
        <Row a="교육·주지 의무" b="작업 시점 안전수칙 고지 타임스탬프 기록" />
      </View>
      ...
      <Text style={[s.h2, { marginTop: 12 }]}>노동자용 현장 증빙 (핵심 신규)</Text>
      <Text style={[s.body]}>
        노동자가 작업 전 위험을 고지받고 점검·서명하거나 위험을 신고하면, 그 행위가 즉시
        불변 로그로 기록됩니다. 노동자 보호 활동이 곧 사업장의 이행 증빙이 되는 구조이며,
        노동자에게 책임을 전가하지 않도록 ‘경영책임자의 의무 이행 입증’ 관점으로 설계했습니다.
      </Text>
      <Footer p="02 · 의무 매핑" />
    </Page>
    ...
    <Page size="A4" style={s.page}>
      ...
      <Num n={2}>위험성평가·작업 전 점검·위험 신고 기록 중 실무상 증거가치가 높은 항목과 보완이 필요한 항목.</Num>
      <Num n={3}>‘상시·연속적 기록’이 일회성 서류 대비 갖는 의미와, 어느 수준까지 갖춰야 유의미한지.</Num>

      <Text style={[s.h3, { marginTop: 8 }]}>B. 법적 리스크</Text>
      <Num n={4}>형식적 이행 리스크: 체크가 형식에 그칠 경우 오히려 불리하게 작용할 가능성과 이를 줄일 설계.</Num>
      <Num n={5}>책임전가 우려: 노동자 행동 기록이 책임을 노동자에게 전가하는 것으로 해석될 여지와 방지책.</Num>
      <Num n={6}>표시·광고 리스크: 마케팅 문구(예: “증빙이 곧 면책”, 요금제·제휴 자료)가 표시광고법상 부당광고에 해당하지 않도록 하는 표현 가이드.</Num>
      <Num n={7}>개인정보·근로감시: 노동자 위치·행동 수집의 적법 요건(동의·목적제한·최소수집), 근로감시 관련 유의점.</Num>
      <Num n={8}>증거능력·무결성: 해시 체인·타임스탬프 기록이 분쟁 시 증거로 활용되기 위한 요건과 보완점.</Num>
    ...
    <Page size="A4" style={s.page}>
      ...
      <Num n={1}>라이브 데모: {CONTACT.web} (자가진단 /diagnosis, 위험성평가 /risk-assessment/new, 노동자 앱 /w)</Num>
      <Num n={2}>샘플 산출물: 위험성평가표 PDF(출처 인용·직인·고지문 포함), 자가진단 갭 리포트</Num>
      <Num n={3}>현장 증빙 구조 설명: 작업 전 점검·위험 신고의 데이터·해시 체인 기록 방식</Num>
